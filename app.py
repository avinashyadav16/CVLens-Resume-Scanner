...
and a question. Please have to provide a clear and concise answer to the question based on the text provided.
...
if 'pdf_qna_session_history' not in st.session_state:
    st.session_state.pdf_qna_session_history = {}


if 'compare_qna_session_history' not in st.session_state:
    st.session_state.compare_qna_session_history = {}
...
if uploaded_file:
    ...
    if resume_id not in st.session_state.pdf_qna_session_history:
        st.session_state.pdf_qna_session_history[resume_id] = {}
    ...
    if len(resume_text) > max_input_length:
        ...
        st.warning(
            "The resume is too long. The text has been truncated to fit within the input limit.")

    # Show a portion of the extracted text
    st.markdown("## Extracted Text:")
    # Display first 1000 characters of the extracted text for reference
    ...
    if st.button("Review Resume 📄"):
        ...
        rating_prompt = rating_prompt_template.format(
            summary=combined_summary, job_description=job_description)
    ...
    if user_question:
        if user_question in st.session_state.pdf_qna_session_history[resume_id]:
            combined_answer = st.session_state.pdf_qna_session_history[resume_id][user_question]
        else:
            question_prompt = question_prompt_template.format(
                resume_text=resume_text, question=user_question)
            combined_answer = generate_gemini_content("", question_prompt)
            st.session_state.pdf_qna_session_history[resume_id][user_question] = combined_answer

        st.markdown("## Answer:")
        st.write(combined_answer)

    # Display previously asked questions and answers for this PDF
    if st.session_state.pdf_qna_session_history[resume_id]:
        st.markdown("## Previously Asked Questions:")
        for question, answer in st.session_state.pdf_qna_session_history[resume_id].items():
...
uploaded_files = st.file_uploader(
    "Upload Resumes [PDF] for comparison", type="pdf", accept_multiple_files=True)

if uploaded_files:
    resume_texts = [extract_text_from_pdf(
        uploaded_file) for uploaded_file in uploaded_files]
    resume_summaries = [generate_gemini_content(
        resume_text, summary_prompt) for resume_text in resume_texts]
    file_names = [uploaded_file.name for uploaded_file in uploaded_files]

    if len(uploaded_files) > 1:
        resume_options = st.multiselect(
            "Select two resumes to compare", file_names, default=file_names[:2])
        if len(resume_options) == 2:
            ...
            if st.button("Compare Resumes"):
                ...
                comparison_prompt = comparison_prompt_template.format(
                    summary1=summary1, summary2=summary2, job_description=job_description)
                ...
                combined_resumes_text = " ".join(
                    [resume_texts[idx1], resume_texts[idx2]])
                combined_resumes_id = generate_pdf_id(combined_resumes_text)

                if combined_resumes_id not in st.session_state.compare_qna_session_history:
                    st.session_state.compare_qna_session_history[combined_resumes_id] = comparison_analysis
                else:
                    comparison_analysis = st.session_state.compare_qna_session_history[
                        combined_resumes_id]
